...
router.get("/get-otp-new", async (req, res) => {
  try {
    const otp = Math.floor(Math.random() * 10000 + 1000);
    console.log(otp);
    client.messages
      .create({
        body: `One Time Password to authorize your action ${otp}`,
        from: myPhone,
        to: req.params.number || "+94761193924",
      })
      .then((message) => {
        const expire = moment(message.dateCreated).add(5, "minutes");
        let otpData = {
...
router.post("/verify-otp-new", async (req, res) => {
    if (tempOTP_New.length > 0) {
      tempOTP_New.map((item, index) => {
        if (req.body.otp === item.otp) {
            tempOTP_New.splice(index, 1);
            const now = new Date().toUTCString();
            const current = moment(now);
            if (item.expire > current) {
              tempOTP_New.splice(index, 1);
